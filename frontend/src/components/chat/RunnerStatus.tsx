...
function summarizeLimits(limits: LiveMandateLimits | undefined): string {
  ...
  if (limits.max_trades_per_day != null) parts.push(`每日 ${limits.max_trades_per_day} 次`);
  if (limits.max_leverage != null) parts.push(limits.max_leverage <= 1 ? "不使用杠杆" : `${limits.max_leverage}× 杠杆`);
  return parts.join(" · ");
}

function fallbackAuthorizeInstruction(): string {
  return "运行 `vibe-trading connector list`，选择券商配置，然后在持有券商连接的桌面会话中运行 `vibe-trading connector authorize <profile>`。";
}
...
function BrokerRow({
  ...
  const authorizeInstruction = authorizeHint?.instruction
    ?? (authorizeFailed ? fallbackAuthorizeInstruction() : "正在加载连接器授权说明...");
  const authorizeNote = "OAuth 成功且授权任务提交前，连接器通道保持只读。";
    ...
    <div className="grid gap-2">
      ...
      >
        ...
        <span className="truncate text-muted-foreground">
          {authorizedCount > 0 ? `${authorizedCount} 个已连接` : "未连接连接器"}
        </span>
        {anyRunning && !isHalted && (
          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[10px] font-medium text-emerald-600 dark:text-emerald-400">
