...
export function Settings() {
    ...
    Promise.all([api.getLLMSettings(), api.getDataSourceSettings()])
      ...
      .catch((error) => {
        ...
        if (isAuthRequiredError(error)) {
          ...
          toast.error(`大模型设置加载失败：${message}`);
          toast.error(`数据源设置加载失败：${message}`);
        }
      })
      .finally(() => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`大模型设置保存失败：${error instanceof Error ? error.message : "未知错误"}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const localApiAccessSection = (
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        ...
        <p className="text-sm text-muted-foreground">{"远程或私有 Web UI 部署时，在当前浏览器中输入一次服务端 API 密钥。本机 localhost 使用可留空。"}</p>
      </div>
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{"配置当前项目的大模型凭据和市场数据源令牌。"}</p>
        </div>
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ...
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? `该提供商使用 OAuth。请运行：${selectedProvider.login_command}`
        : "该提供商不需要 API 密钥。";
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">{"选择智能体使用的大模型，并保存到项目本地的 agent/.env 文件。"}</p>
      </div>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{"切换提供商会同步更新推荐模型和接口地址。"}</span>
            </label>
            ...
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{"请填写提供商要求的精确模型 ID。"}</span>
            </label>

            <label className="grid gap-2">
              <span className={labelClass}>{"基础 URL"}</span>
              <input
                value={form.base_url}
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{"控制模型回答前的推理投入。越高越充分但更慢；需要最快回复时保持关闭。"}</span>
            </label>
      ...
      <form onSubmit={submitDataSources} className="rounded-lg border bg-card p-5 shadow-sm">
        <div className="mb-5 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">{"配置回测和研究智能体使用的可选市场数据凭据。"}</p>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{"用于 A 股、期货、基金和宏观数据。未配置时，项目会在可用场景下回退到 AKShare。"}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
          ...
          <div className="rounded-md border bg-muted/20 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <span className={`rounded-full px-2 py-0.5 text-xs ${dataSettings.baostock_supported ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
                {dataSettings.baostock_supported ? "加载器可用" : "项目未配置加载器"}
              </span>
            </div>
